...
export const EditTrade: FC<CollectionChangeProps> = ({collectionDetails,verifiedCollection, program}) => {
    ...
    const onSubmit = useCallback(async (e) => {
        ...
        if (editType["duration"]) {
            if (num % 1) {
                notify({ type: 'error', message: `Duration cannot be in decimals` });
                return;
            }
        }
        ...
        if (editType["fee"]) {
            newNum = newNum * LAMPORTS_PER_SOL;
        }
        ...
        try {
            signature = await program.methods.editTrade(
    ...
    const backClick = () => {
        ...
        document.querySelector(`.edit-trade-module`).classList.add('hidden');
    }
        ...
        <form onSubmit={onSubmit}>
            ...
            <div className="mb-6">
                <div className="mr-4 inline">
                    <input onClick={handleClick} id="orange-radio" type="radio" value="fee" name="edit-type" className="w-4 h-4 text-orange-500 bg-gray-100 border-gray-300 focus:ring-orange-500 dark:focus:ring-orange-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" />
                    <label htmlFor="orange-radio" className="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">Trade Fee</label>
                </div>
                <div className="mr-4 inline">
